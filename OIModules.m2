...
OIModule OIObject := Module => (M,n) -> (
	...
	gensmat := if (phi =!= null and (phi n) != 0) then (phi n)
	else id_nthModule;
	relsmat := if (psi =!= null and (psi n) != 0) then (psi n)
	else map(nthModule,R^0,0); 
...
OIgb(OIModule) := M -> (
	...
	inMk := initialVect Mkgb; -- initial terms of M k
	
	inG := apply(G, g->initialVect g); -- initial terms of G
    	inGmap := oiModuleMap(target Mgens, A^widths, inG);
	inGk := inGmap k; -- what inG generates in width k
	inGk = apply(numcols inGk, j->inGk_{j});
	-- check if each initial term of M k is generated by inG.
	-- if not, it's added to G
	for i from 0 to (numcols inMk)-1 do (
	    v := inMk_{i};
	    if not any(inGk, w->w==v) then (
		G = append(G,Mkgb_{i});
